...
export default function StudentLogin() {
  ...
  async function login() {
    setError('')
    const { valid } = validateCollegeDomain(email)
    if (!valid) { setError('Use your official college email address.'); return }
    setLoading(true)
    const { error: e } = await supabase.auth.signInWithOtp({ email, options: { emailRedirectTo: `${window.location.origin}/student/onboard` } })
    setLoading(false)
  ...
  if (sent) return (
    <div className="min-h-screen bg-navy flex items-center justify-center p-4">
      <div className="bg-[#F5F0E8] rounded-2xl p-10 max-w-sm w-full text-center">
        ...
        <div className="text-sm text-steel">Login link sent to <strong>{email}</strong></div>
      </div>
    </div>
  )
  ...
  return (
    <div className="min-h-screen bg-navy flex items-center justify-center p-4">
      <div className="bg-[#F5F0E8] rounded-2xl p-10 max-w-sm w-full">
        ...
        <button onClick={login} disabled={loading || !email} className="w-full py-2.5 bg-navy text-cream text-sm font-medium rounded-lg disabled:opacity-50">
...
function FPMark() {
  return (
    <div className="w-12 h-12 rounded-full bg-navy flex items-center justify-center mx-auto">
      <svg width="22" height="19" viewBox="0 0 229.163 195.391"><path fill="#64ade0" d="M44.444,94.164l42.268,42.317,58.87-54.823-32.514,88.243h71.651V25.491L44.444,94.164Z"/></svg>
    </div>
  )
}
